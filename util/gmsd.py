...
def gmsd(
    ...
    score, mean_gms = _gmsd(x=x, y=y, t=t)
    return _reduce(score, reduction), _reduce(mean_gms, reduction)


def _gmsd(
    x: torch.Tensor, y: torch.Tensor, t: float = 170 / (255.0**2), alpha: float = 0.0
) -> torch.Tensor:
    r"""Compute Gradient Magnitude Similarity Deviation
...
class GMSDLoss(_Loss):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        return gmsd(
            x=x, y=y, reduction=self.reduction, data_range=self.data_range, t=self.t
        )
...
def multi_scale_gmsd(
    ...
    if scale_weights is None:
        scale_weights = torch.tensor(
            [0.096, 0.596, 0.289, 0.019], device=x.device, dtype=x.dtype
        )
    else:
