...
__global__ void intersect_tile_rectangular_kernel(
    ...
    float tile_radius_x = radius_x / static_cast<float>(tile_size_x);
    float tile_radius_y = radius_y / static_cast<float>(tile_size_y);
    float tile_x = mean2d.x / static_cast<float>(tile_size_x);
    float tile_y = mean2d.y / static_cast<float>(tile_size_y);
...
void launch_intersect_tile_rectangular_kernel(
    // inputs
    ...
    AT_DISPATCH_FLOATING_TYPES(
        means2d.scalar_type(),
        "intersect_tile_rectangular_kernel",
        [&]() {
            intersect_tile_rectangular_kernel<scalar_t>
                <<<grid,
                   threads,
                   shmem_size,
...
__global__ void intersect_offset_rectangular_kernel(
...
void launch_intersect_offset_rectangular_kernel(
    // inputs
    ...
    uint32_t n_tiles = tile_width * tile_height;
    uint32_t tile_n_bits = (uint32_t)floor(log2(n_tiles)) + 1;
    intersect_offset_rectangular_kernel<<<
        grid,
        threads,
        shmem_size,
        at::cuda::getCurrentCUDAStream()>>>(
...
void radix_sort_double_buffer_rectangular(
...
// DoubleBuffer reduce the auxiliary memory usage from O(N+P) to O(P)
void segmented_radix_sort_double_buffer_rectangular(
    const int64_t n_isects,
