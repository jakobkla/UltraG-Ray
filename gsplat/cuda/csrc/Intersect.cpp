...
std::tuple<at::Tensor, at::Tensor, at::Tensor> intersect_tile_rectangular(
    ...
    at::Tensor offsets;
    if (n_elements) {
        launch_intersect_tile_rectangular_kernel(
            // inputs
            means2d,
            radii,
            depths,
    ...
    if (n_isects && sort) {
        ...
        at::Tensor flatten_ids_sorted = at::empty_like(flatten_ids);
        if (segmented) {
            segmented_radix_sort_double_buffer_rectangular(
                n_isects,
                I,
                image_n_bits,
                tile_n_bits,
            ...
            radix_sort_double_buffer_rectangular(
                n_isects,
                image_n_bits,
                tile_n_bits,
                isect_ids,
...
at::Tensor intersect_offset_rectangular(
    ...
    launch_intersect_offset_rectangular_kernel(
        isect_ids, I, tile_width, tile_height, offsets
    );
    return offsets;
}
